...
| 29 | **Found by reading the script's own stderr, not by a review:** `assert_text_fits` measured on R's default pdf device, which cannot encode the em-dashes these titles use — it printed `conversion failure … dot substituted` and measured a NARROWER string than the PNG renders, i.e. the guard against text running off the page was measuring text the page does not contain, in the direction that lets a clipped title pass. Now measured on a cairo device. |
